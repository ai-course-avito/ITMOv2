...
| Асинхронная обработка через очередь | Снимает удержание воркера на 10 секунд, но растягивает время до готового разбора: ревьюер получает результат не сразу, а после обработки задачи из очереди. Кроме того появляется ещё одно место, где содержимое diff лежит на диске, и его тоже придётся защищать по `SEC-1`. Вернуться к вопросу, если синхронный вызов упрётся в пропускную способность |
...
- Тип промпта: Tree of Thoughts для перебора альтернатив, R.C.T.F. для остального
- Строка в [`prompts.md`](prompts.md): `P1-06`
- Что проверили и исправили сами: сверил решение и все четыре альтернативы с правилами `CASE.md`. Забраковал обоснование отказа от очереди: `OBS-1` — правило про логи, а не про хранилище, подмена заменена на реальные минусы очереди. Статус оставил `proposed`, поскольку ни одна проверка из плана ещё не выполнена. Убрал из контракта натяжку про перенос вытесненных нарушений в `checks`, оставив потолок `OUT-1` как есть
