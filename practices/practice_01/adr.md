...
| Самописные регулярные выражения (regex) для SEC-1 | Высокий риск пропуска секретов, сложно поддерживать базу паттернов актуальной |
| Выделенный сайдкар-прокси для фильтрации секретов | DevOps-оверхед, усложнение инфраструктуры для MVP |

*Примечание по SEC-1:* Для очистки секретов выбран гибридный подход — использование быстрого пред-фильтра (regex на базовые вещи) + вызов встроенной библиотеки `gitleaks` (или аналога) в detect-only режиме для высокой надежности (см. P2-04 Tree of Thoughts).
...
- Строка в [`prompts.md`](prompts.md): P1-01 (zero-shot, выявление архитектурных рисков), P2-04 (Tree of Thoughts, выбор реализации SEC-1).
- Что проверили и исправили сами: каждое решение сопоставлено с правилом из `CASE.md`; с помощью Tree of Thoughts глубоко проанализировали реализацию очистки секретов, отказавшись от наивных регулярок в пользу интеграции с Gitleaks; Mermaid-схема построена вручную и сверена с правилами.
