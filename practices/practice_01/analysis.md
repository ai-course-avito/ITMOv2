...
flowchart LR
    ...
    LLM -- OK --> OUT{Проверка\nOUT-1}
    OUT -- валиден --> Resp["{ summary, risks≤3, checks }"]
    OUT -- невалиден --> Err422[HTTP 422\nнарушение OUT-1]
...
- Строка в [`prompts.md`](prompts.md): P1-01 (zero-shot), P2-06 (ReAct, аудит блок-схемы).
- Что проверили и исправили сами: Mermaid-схема изначально построена вручную по правилам `CASE.md`; с помощью ReAct выявлен логический пробел — отсутствие валидации ответа от LLM (`OUT-1`). Схема обновлена, чтобы явно показывать возврат `HTTP 422` при некорректном JSON.
