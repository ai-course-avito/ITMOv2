...
| Секреты внутри diff | уходят во внешний сервис как есть, строка `review_service.py:20` | удаляются до вызова LLM по `SEC-1` | передать diff с тестовым токеном и убедиться, что в промпте остался `[REDACTED]` |
...
- Что проверили и исправили сами: решил оставить одну схему вместо двух, чтобы не поддерживать дубликат при правках. Потребовал перенести подпись HTTP 413 под стрелку, иначе линия её перечёркивала. Проверил рендер в VS Code
