...
|---|---|---|---|---|---|---|
| Базовая нагрузка на `/health` | **Окружение:** FastAPI/Uvicorn, 4 worker-процесса, 1 vCPU, 2 GB RAM (контейнер/CI).<br>**LLM:** Не вызывается (проверка liveness/readiness). | 100 RPS в течение 60 секунд. | **p99 < 20 мс**, 0% ошибок (HTTP 200).<br>*Обоснование:* Эндпоинт отдает статический статус из памяти; 4 асинхронных воркера на 1 vCPU удерживают задержку p99 < 20 мс при 100 RPS. | При повышении нагрузки до 200 RPS время ответа деградирует плавно без 5xx и падения процесса (OOM/crash). | CPU utilization < 40%, RSS RAM процесса < 150 MB. | `k6 run --vus 10 --duration 60s -e SCENARIO=health load/load_tests.js` |
| Потоковая отправка валидных diff | **Окружение:** FastAPI/Uvicorn, 4 worker-процесса, 1 vCPU, 2 GB RAM.<br>**LLM:** Замокан локальной заглушкой с задержкой 200 мс (без вызова внешнего API и расхода токенов). | 10 одновременных пользователей (VUs), diff 5 000 символов (в рамках `API-1`), 120 секунд. | Время ответа сервиса **< 10.5 с** (порог `REL-1` таймаута LLM 10 с + 500 мс на маскирование по `SEC-1` и валидацию `OUT-1`), 0% ошибок 5xx. При моке 200 мс: **p95 < 400 мс**.<br>*Обоснование:* 4 воркера в асинхронном event-loop обслуживают 10 параллельных non-blocking вызовов mock-LLM. | При искусственной задержке mock-LLM в 11 с (> лимита `REL-1` 10 с) сервис штатно обрывает запрос по таймауту 10 с и отдает контролируемый ответ (HTTP 504), воркеры не зависают, соединения закрываются. | CPU utilization < 70%, RSS RAM процесса < 350 MB (без утечек памяти на маскировании `SEC-1`). | `k6 run --vus 10 --duration 120s -e SCENARIO=diff -e MOCK_LLM_DELAY_MS=200 load/load_tests.js`<br>Тест деградации по REL-1: `k6 run --vus 10 --duration 60s -e SCENARIO=diff -e MOCK_LLM_DELAY_MS=11000 load/load_tests.js` |
| Стресс-тест отсечки `API-1` (граничные значения) | **Окружение:** FastAPI/Uvicorn, 4 worker-процесса, 1 vCPU, 2 GB RAM.<br>**LLM:** Замокан, вызовы с превышением лимита блокируются валидацией сервиса (0 обращений к LLM). | Смешанная нагрузка 50 RPS, 60 секунд: граничные валидные (20 000 символов) и невалидные (20 001 символ) payload. | **p99 < 50 мс**.<br>Для diff <= 20 000 символов: 100% HTTP 200 (при замоканном LLM).<br>Для diff == 20 001 символ: 100% HTTP 413 (Payload Too Large).<br>*Обоснование:* Валидация длины diff (`API-1`) выполняется до вызова `ReviewService` и маскирования `SEC-1`. | При всплеске до 100 RPS оверзайз-диффов сервис моментально реджектит запросы с кодом 413, защищая CPU/RAM от перегрузки и не блокируя параллельный трафик. | CPU utilization < 50%, прирост RSS RAM < 50 MB (быстрое освобождение буфера запроса). | Валидные 20 000: `k6 run --vus 20 --duration 60s -e SCENARIO=boundary_valid load/load_tests.js`<br>Невалидные 20 001: `k6 run --vus 20 --duration 60s -e SCENARIO=boundary_invalid load/load_tests.js` |
...
export default function () {
  ...
  } else if (SCENARIO === 'boundary_valid') {
    const validPayload = JSON.stringify({
      diff: 'x'.repeat(20000), // ровно граница API-1
    });
    const res = http.post(`${BASE_URL}/api/reviews`, validPayload, {
      headers: { 'Content-Type': 'application/json' },
    });
    check(res, {
      'status is 200': (r) => r.status === 200,
      'response time < 10500ms': (r) => r.timings.duration < 10500,
    });
  } else if (SCENARIO === 'boundary_invalid') {
    const oversizePayload = JSON.stringify({
      diff: 'x'.repeat(20001), // строго больше 20 000 по API-1
    });
...
- Эксперимент и промпт: [`experiment.md`](experiment.md) (техника Chain of Verification, строка Chain of Verification в [`../prompts.md`](../prompts.md)).
- Исходный артефакт: `practices/practice_01/tests_load.md` (создан по строке P1-03 в `practices/practice_01/prompts.md`).
- Что проверили и исправили сами: сформулировали 6 проверочных вопросов, заменили 25 000 символов на граничные тесты 20 000 / 20 001 по API-1, обосновали порог < 10.5 с по tests_integration.md и зафиксировали мок LLM со сценарием деградации.
